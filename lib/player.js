...
Player.prototype.replaceWithFavorite = function (favorite, callback) {
  ...
  player.getFavorites(function (success, favorites) {
    favorites.forEach(function (item) {
      if (item.title.toLowerCase().indexOf(decodeURIComponent(favorite).toLowerCase()) == 0) {
        ...
        player.removeAllTracksFromQueue(function (success) {
          ...
          var targetShuffle = item.title.toLowerCase().indexOf("(s)", item.title.length - 3) !== -1;
          var targetPlayMode = targetShuffle ? "SHUFFLE_NOREPEAT" : "NORMAL";
          player.setPlayMode(targetPlayMode, function(success) {
            if (!success) {
              console.log("problem setting play mode");
              callback(false);
              return;
            }
